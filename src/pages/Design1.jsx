    ...
    >
      ...
      <nav className="fixed top-0 left-0 right-0 z-50">
        <div className="max-w-6xl mx-auto px-6 md:px-10 py-5 flex items-center justify-between">
          <Link
            to="/"
            className="text-xs tracking-[0.2em] uppercase text-[#2D2A26]/40 hover:text-[#2D2A26] transition-colors"
          >
            &larr; Zurück
          </Link>
      ...
      <section className="min-h-screen flex items-center justify-center px-6 md:px-10 pt-20 relative">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p
            className={`text-lg md:text-xl text-[#2D2A26]/50 max-w-xl mx-auto leading-relaxed mb-12 transition-all duration-1000 delay-400 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
          >
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              <div
                key={i}
                className="bg-white/40 backdrop-blur-sm border border-[#2D2A26]/5 rounded-2xl p-6 text-center hover:bg-white/60 hover:shadow-lg hover:shadow-[#C17E4F]/5 transition-all duration-500"
              >
                <div
                  style={{ fontFamily: "'Instrument Serif', serif" }}
                ...
                <div className="text-[#2D2A26]/35 text-xs tracking-wider uppercase">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {process_steps.map((step, i) => (
              <div
                key={i}
                className="group bg-white/30 backdrop-blur-sm border border-[#2D2A26]/5 rounded-3xl p-8 md:p-10 hover:bg-white/60 hover:shadow-xl hover:shadow-[#C17E4F]/5 hover:-translate-y-1 transition-all duration-500"
              >
                <div className="flex items-start gap-5">
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#D4956A]/20 to-[#C17E4F]/10 flex items-center justify-center text-[#C17E4F] shrink-0 group-hover:scale-110 transition-transform duration-500">
                    {step.icon}
                  </div>
                  <div>
                    <div className="flex items-center gap-3 mb-3">
                      <span className="text-[#2D2A26]/15 text-xs font-mono">0{i + 1}</span>
                      <h3 className="text-xl font-semibold">{step.title}</h3>
                    </div>
                    <p className="text-[#2D2A26]/45 leading-relaxed">{step.desc}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {['Next.js', 'React', 'TailwindCSS', 'shadcn/ui', 'Bun', 'Vercel', 'TypeScript', 'Lucide'].map(
              (tech) => (
                <span
                  key={tech}
                  className="px-5 py-2.5 bg-white/40 backdrop-blur-sm border border-[#2D2A26]/5 rounded-full text-sm text-[#2D2A26]/60 hover:bg-[#2D2A26] hover:text-[#FFF8F0] transition-all duration-300 cursor-default"
                >
                  {tech}
                </span>
              )
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto">
            {[
              ...
              <div
                key={i}
                className="bg-white/40 backdrop-blur-sm border border-[#2D2A26]/5 rounded-3xl p-8 hover:bg-white/60 hover:shadow-xl transition-all duration-500"
              >
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#D4956A] to-[#C17E4F] flex items-center justify-center text-white text-lg font-bold mb-5">
                  {person.name.split(' ').map(n => n[0]).join('')}
                </div>
                <h3 className="text-xl font-semibold mb-1">{person.name}</h3>
                <p className="text-[#C17E4F] text-sm mb-3">{person.role}</p>
                <p className="text-[#2D2A26]/40 text-sm leading-relaxed">{person.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-32 px-6 md:px-10">
        <div className="max-w-3xl mx-auto text-center">
          <div className="bg-gradient-to-br from-[#2D2A26] to-[#3D3A36] rounded-[2rem] p-12 md:p-20 text-[#FFF8F0] relative overflow-hidden">
            <div className="absolute top-0 right-0 w-64 h-64 bg-[#C17E4F] rounded-full opacity-10 blur-[80px] -translate-y-1/2 translate-x-1/2" />
            <h2
              style={{ fontFamily: "'Instrument Serif', serif" }}
            ...
            <p className="text-[#FFF8F0]/40 mb-10 max-w-md mx-auto relative">
              Kostenlose Erstberatung. Unverbindlich. Wir freuen uns auf Ihr Projekt.
            </p>
            <a
              href="mailto:hello@bm-web.de"
              className="group inline-flex items-center gap-3 bg-gradient-to-r from-[#C17E4F] to-[#D4956A] text-white px-8 py-4 rounded-2xl text-sm font-medium hover:shadow-2xl hover:shadow-[#C17E4F]/30 hover:-translate-y-0.5 transition-all duration-300 relative"
            >
